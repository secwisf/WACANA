...
class EosEmulator:
    ...
    @classmethod
    def eosio_assert(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        def gets(m,a):
            res=""
            while a<len(m.data) and m.data[a]!=0:
                res+=chr(m.data[a])
                a+=1
            return res
        expression = args[0].n
        ...
        states[0].add_constraint(expression == 1)
        # print(expression)
        # print(solver)
        if global_vars.cut_on_unsat and solver.check() == z3.unsat:
            logger.infoln('assert error')
            m=store.mems[module_instance.memaddrs[0]]
            logger.infoln('output',gets(m,args[1].n))
    ...
    @classmethod
    def tapos_block_num(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        return utils.gen_symbolic_value(bin_format.i32, f'tapos_block_num_{next(cls.counter)}'),states
    ...
    @classmethod
    def current_time(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        # return utils.gen_symbolic_value(bin_format.i64, f'current_time_{next(cls.counter)}')
        res=int(time.time()*1000000+300000)
        return res,states
    ...
    @classmethod
    def action_data_size(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        if global_vars.test_argument_type:
            res=0
            for i,ta in enumerate(global_vars.test_argument_type):
                if ta=='string' or ta=='struct':
                    res+=10
                elif ta=='name' or '64' in ta:
                    res+=8
                elif '32' in ta:
                    res+=4
                else:
            ...
            return utils.gen_symbolic_value(bin_format.i32, f'action_data_size_{next(cls.counter)}'),states

    @classmethod
    def current_receiver(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        return utils.eos_abi_to_int(global_vars.contractName),states
        return utils.gen_symbolic_value(bin_format.i64, f'current_receiver_{next(cls.counter)}')

    @classmethod
    def has_auth(cls, args, solver, store, module_instance):
        state=[GlobalState()]
        return utils.gen_symbolic_value(bin_format.i64, f'has_auth_{next(cls.counter)}'),state
        # return utils.gen_symbolic_value(bin_format.i32, f'has_auth_{next(cls.counter)}')

    @classmethod
    def is_account(cls, args, solver, store, module_instance):
        state=[GlobalState()]
        return utils.gen_symbolic_value(bin_format.i64, f'is_account_{next(cls.counter)}'),state
    ...
    @classmethod
    def read_action_data(cls, args, solver, store, module_instance):
        src=args[0].n
        m=store.mems[module_instance.memaddrs[0]]
        if global_vars.test_argument_type:
            cnt=src
            for i,v in enumerate(global_vars.test_argument_type):
                if v=='string' or v=='struct':
                    m[cnt:cnt+10]='a'*10
                    cnt+=10
                elif v=='name':
                    m.store(cnt,8 , number.MemoryStore.pack_i32(utils.eos_abi_to_int(global_vars.test_argument[i])))
                    cnt+=10
                elif '32' in v: 
                    m.store(cnt,4,number.MemoryStore.pack_i32(utils.gen_symbolic_value(bin_format.i32,f'action_arg_{i}')))
                    cnt+=4
                elif '64' in v: 
                    m.store(cnt,8,number.MemoryStore.pack_i64(utils.gen_symbolic_value(bin_format.i64,f'action_arg_{i}')))
                    cnt+=8
    ...
    @classmethod
    def memcpy(cls, args, solver, memory):
        #TODO:这里之后要加上写Rule
        (dest,src,length)=args
        memory.data[dest.n:dest.n+length.n]=memory.data[src.n:src.n+length.n]
        return dest.n
    ...
    @classmethod
    def require_auth(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        if utils.is_bv_concrete(name):
            return True,states
        name = args[0].n
        solver.push()
        solver.add(name==utils.eos_abi_to_int(global_vars.contractName))
        states[0].add_constraint(name==utils.eos_abi_to_int(global_vars.contractName))
        return None,states

    @classmethod
    def require_auth2(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        if utils.is_bv_concrete(name):
            return True,states
        name = args[0].n
    ...
    def activate_feature(cls, ctx, args):
        pass
        # cls.eosio_assert(cls, ctx, args[0])

    @classmethod
    def db_store_i64(cls, args, solver, store, module_instance):
        scope,table,payer,id,addr,len=args
        m=store.smems[module_instance.memaddrs[0]]
        data=m.data[addr.n:addr.n+len.n]
        global_vars.storage.store(scope.n,table.n,payer.n,id.n,data.n,len.n)


    @classmethod
    def db_update_i64(cls, args, solver, store, module_instance):
        pass

    @classmethod
    def db_remove_i64(cls, args, solver, store, module_instance):
        pass

    @classmethod
    def db_get_i64(cls, args, solver, store, module_instance):
        #根据索引获得具体的值，并且写入到第二个参数对应的缓冲区中
        #iter是符号量的话，需要添加额外的约束，因为认为是iter.n=0
        states=[GlobalState()]
        iter,data,target_len=args
        storage_bin=global_vars.storage.get_bin(global_vars.contractName,iter.n,states)
        # storage=global_vars.storage.get_json(iter)
        if storage_bin is None:
            return -1,states
        data_len=len(storage_bin)//2
        if target_len.n==0 or data.n==0:
            return data_len,states
        if utils.is_bv_concrete(data.n):
            data.n=utils.get_concrete_int(data.n)
            m=store.mems[module_instance.memaddrs[0]]
            for i in range(data_len):
                m.data[data.n+i:data.n+i+1]=number.MemoryStore.pack_u8(int(storage_bin[2*i:2*i+2],base=16))
        else:
            m=store.smems[module_instance.memaddrs[0]]
            for i in range(data_len):
                m.store(data.n+i,1,number.MemoryStore.pack_u8(int(storage_bin[2*i:2*i+2],base=16)))
        return data_len,states

    @classmethod
    def db_next_i64(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        storage=global_vars.storage
        if storage is None:
            return -2,states
        iter,primary=args
        if utils.is_bv_concrete(iter.n):
            iter.n=utils.get_concrete_int(iter.n)
        else:
            for s in states:
                s.add_constraint(iter.n==0)
            iter.n=0

        if utils.is_bv_concrete(primary.n):
            primary.n=utils.get_concrete_int(primary.n)
            m=store.smems[module_instance.memaddrs[0]]
        else:
            m=store.mems[module_instance.memaddrs[0]]
            
        if 0<=iter.n<len(storage)-1:
            next_iter=iter.n+1
            if utils.is_bv_concrete(primary.n):
                m.data[primary.n:primary.n+8]=number.MemoryStore.pack_i64(storage.get_key_by_iter(next_iter))
            else:
                m.store(primary.n,8,number.MemoryStore.pack_i64(storage.get_key_by_iter(next_iter)))
            return next_iter,states
        elif iter.n==len(storage)-1:
            return -2,states
        else:
            return -1,states
    ...
    @classmethod
    def db_previous_i64(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        storage=global_vars.storage
        if storage is None:
            return -2,states
        ...
        if utils.is_bv_concrete(iter.n):
            ...
            for s in states:
                s.add_constraint(iter.n==global_vars.storage.iters[len(global_vars.storage)-1])
            iter.n=global_vars.storage.iters[len(global_vars.storage)-1]
        if utils.is_bv_concrete(primary.n):
            primary.n=utils.get_concrete_int(primary.n)
            m=store.mems[module_instance.memaddrs[0]]
        else:
            m=store.smems[module_instance.memaddrs[0]]
        if 0<iter.n<len(storage):
            previous_iter=iter.n-1
            if utils.is_bv_concrete(primary.n):
                m.data[primary.n:primary.n+8]=number.MemoryStore.pack_i64(storage.get_key_by_iter(previous_iter))
            else:
                m.store(primary.n,8,number.MemoryStore.pack_i64(storage.get_key_by_iter(previous_iter)))
            return previous_iter,states
        elif iter.n==0:
            return -2,states
        else:
            return -1,states
    ...
    @classmethod
    def db_find_i64(cls, args, solver, store, module_instance):
        # print(args)
        states=[GlobalState()]
        #获取一个索引
        print(list(map(lambda x:utils.is_bv_concrete(x.n),args[:3])))#应该是contract_name,contract_name,table_name
        return global_vars.storage.find(*list(map(lambda x:x.n,args[:4]))),states

    @classmethod
    def db_idx64_find_primary(cls, ctx, args):
        return utils.gen_symbolic_value(bin_format.i32, f'db_idx64_find_primary_{next(cls.counter)}')

    @classmethod
    def db_lowerbound_i64(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        code,scope,table,id=args
        return global_vars.storage.lowerbound(code.n,scope.n,table.n,id.n,states),states


    @classmethod
    def db_upperbound_i64(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        code,scope,table,id=args
        return global_vars.storage.upperbound(code.n,scope.n,table.n,id.n,states),states

    @classmethod
    def db_end_i64(cls, args, solver, store, module_instance):
        states=[GlobalState()]
        code,scope,table=args
        return global_vars.storage.end(code.n,scope.n,table.n),states
    ...
    @classmethod
    def send_inline(cls, args, solver, memory):
        (data,length)=args
...
hostfunc_map = {
    'env': {
        ...
        'db_store_i128': EosEmulator.db_store_i64,
        'db_store_i256': EosEmulator.db_store_i64,
        'db_idx64_store': EosEmulator.db_store_i64,
        'db_idx128_store': EosEmulator.db_store_i64,
        'db_idx256_store': EosEmulator.db_store_i64,
        'db_idx_double_store': EosEmulator.db_store_i64,
        'db_update_i64': EosEmulator.db_update_i64,
        'db_update_i128': EosEmulator.db_update_i64,
        'db_update_i256': EosEmulator.db_update_i64,
        'db_idx64_update': EosEmulator.db_update_i64,
        'db_idx128_update': EosEmulator.db_update_i64,
        'db_idx256_update': EosEmulator.db_update_i64,
        'db_idx_double_update': EosEmulator.db_update_i64,
        'db_remove_i64': EosEmulator.db_remove_i64,
        'db_remove_i128': EosEmulator.db_remove_i64,
        'db_remove_i256': EosEmulator.db_remove_i64,
        'db_idx64_remove': EosEmulator.db_remove_i64,
        'db_idx128_remove': EosEmulator.db_remove_i64,
        'db_idx256_remove': EosEmulator.db_remove_i64,
        'db_idx_double_remove': EosEmulator.db_remove_i64,
        'db_get_i64': EosEmulator.db_get_i64,
        'db_get_i128': EosEmulator.db_get_i64,
        'db_get_i256': EosEmulator.db_get_i64,
        'db_idx64_get': EosEmulator.db_get_i64,
        'db_idx128_get': EosEmulator.db_get_i64,
        'db_idx256_get': EosEmulator.db_get_i64,
        'db_idx_double_get': EosEmulator.db_get_i64,
        'db_next_i64': EosEmulator.db_next_i64,
        'db_next_i128': EosEmulator.db_next_i64,
        'db_next_i256': EosEmulator.db_next_i64,
        'db_idx64_next': EosEmulator.db_next_i64,
        'db_idx128_next': EosEmulator.db_next_i64,
        'db_idx256_next': EosEmulator.db_next_i64,
        'db_idx_double_next': EosEmulator.db_next_i64,
        'db_previous_i64': EosEmulator.db_previous_i64,
        'db_previous_i128': EosEmulator.db_previous_i64,
        'db_previous_i256': EosEmulator.db_previous_i64,
        'db_idx64_previous': EosEmulator.db_previous_i64,
        'db_idx128_previous': EosEmulator.db_previous_i64,
        'db_idx256_previous': EosEmulator.db_previous_i64,
        'db_idx_double_previous': EosEmulator.db_previous_i64,
        'db_find_i64': EosEmulator.db_find_i64,
        'db_find_i128': EosEmulator.db_find_i64,
        'db_find_i256': EosEmulator.db_find_i64,
        'db_idx64_find_primary': EosEmulator.db_idx64_find_primary,
        'db_idx128_find_primary': EosEmulator.db_idx64_find_primary,
        'db_idx256_find_primary': EosEmulator.db_idx64_find_primary,
        'db_idx_double_find_primary': EosEmulator.db_idx64_find_primary,
        'db_lowerbound_i64': EosEmulator.db_lowerbound_i64,
        'db_lowerbound_i128': EosEmulator.db_lowerbound_i64,
        'db_lowerbound_i128': EosEmulator.db_lowerbound_i64,
        'db_idx64_lowerbound': EosEmulator.db_lowerbound_i64,
        'db_idx128_lowerbound': EosEmulator.db_lowerbound_i64,
        'db_idx256_lowerbound': EosEmulator.db_lowerbound_i64,
        'db_idx_double_lowerbound': EosEmulator.db_lowerbound_i64,
        'db_upperbound_i64': EosEmulator.db_upperbound_i64,
        'db_upperbound_i128': EosEmulator.db_upperbound_i64,
        'db_upperbound_i128': EosEmulator.db_upperbound_i64,
        'db_idx64_upperbound': EosEmulator.db_upperbound_i64,
        'db_idx128_upperbound': EosEmulator.db_upperbound_i64,
        'db_idx256_upperbound': EosEmulator.db_upperbound_i64,
        'db_idx_double_upperbound': EosEmulator.db_upperbound_i64,
        'db_end_i64': EosEmulator.db_end_i64,
        'db_end_i128': EosEmulator.db_end_i64,
        'db_end_i256': EosEmulator.db_end_i64,
        'db_idx64_end': EosEmulator.db_end_i64,
        'db_idx128_end': EosEmulator.db_end_i64,
        'db_idx256_end': EosEmulator.db_end_i64,
        'db_idx_double_end': EosEmulator.db_end_i64,
...
realize_list_host = ['callDataCopy','storageStore','storageLoad','getCaller',
                ...
                'getBlockNumber', 'getExternalCodeSize', 'getExternalBalance', 'memcpy']
#需要添加memcpy,action_data_size,eosio_assert
