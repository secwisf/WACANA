...
from fuzz import clear_env, get_action_info,exec_on_chain
from onchain_storage import Storage
import utils
import json
import traceback
import time
...
def locate_transfer(vm, name):
    """Using specific parameters to locate [on_]transfer function.
...
def build_model(vm, name) -> None:
    ...
    action_to_func_id,action_to_args,store_logs,feedbackFactory=get_action_info(f"./datasets/vul_dataset/{name}/{name}.abi",f"./datasets/vul_dataset/{name}/{name}.wasm")
    if global_vars.apply_function_address is None:
        return

    # global_vars.fake_detect()  # set flag for fake detection
    func_id_to_action=dict(map(lambda t:(t[1],t[0]),action_to_func_id.items()))
    func_id_to_params={}
    #建模每个action
    for action_name in action_to_func_id.keys():
        vm.reinit()
        global_vars.should_write_rule=True
        global_vars.contractName=name
        global_vars.BRANCH_DEPTH_LIMIT=200
        global_vars.cut_on_unsat=False
        print("model",action_name)
        ...
        init_constraints = []
        params[0]=utils.gen_symbolic_value(bin_format.i32,'addr_0')
        symbolic_memory=vm.store.smems[vm.module_instance.memaddrs[0]]
        # print(utils.eos_abi_to_int(name))
        symbolic_memory.store(params[0],16,number.MemoryStore.pack_i64(utils.eos_abi_to_int(name))*2)
        ...
        for i,ta in enumerate(test_argument_type):
            if ta=='string' or ta=="struct":
                params[i+1]=utils.gen_symbolic_value(bin_format.i32,f'addr_{i}')
            if ta=='string':
                tas=test_argument_dict[test_argument_name[i]]
                symbolic_memory.store(params[i+1],len(tas),list(map(ord,tas)))
        global_vars.test_argument_type=test_argument_type
        global_vars.test_argument=[test_argument_dict[i] for i in test_argument_name]
        try:
            r,states,_=vm.exec_by_address(vm.module_instance.funcaddrs[action_to_func_id[action_name]], params, init_constraints)
        except Exception as e:
            logger.infoln(f'Exception {e}')
            traceback.print_exc()
            states=[]
        # all_states.extend([s for s in states if s.write_storage])
    ##建模响应函数之前的内容
    #vm.reinit()
    #global_vars.contractName=name
    #global_vars.BRANCH_DEPTH_LIMIT=10
    #global_vars.cut_on_unsat=False
...
def detect_permissions(vm, name) -> None:
    """Fake eos transfer vulnerability analysis function.
    ...
    if apply_func.functype == func_type:
        ...
        params[0] = utils.eos_abi_to_int(name)
        init_constraints = []
        r=vm.exec_by_address(global_vars.apply_function_address, params, init_constraints)
        print(r)
    # global_vars.sym_exec() # set the detection mode to False

# @profile
def symbolic_loop(vm, name, data_dir):
    global_vars.contractName=name

    global_vars.user_name=name.split('_')[-1] if '_' in name else name
    action_to_func_id,action_to_args,store_logs,feedbackFactory=get_action_info(f"{data_dir}/{name}/{name}.abi",f"{data_dir}/{name}/{name}.wasm")
    # print('--------------------------------------------------------')
    # print(f"{data_dir}/{name}/{name}.abi")
    print("storage log",store_logs)
    feedbackFactory.processed_logs=list()
    clear_env()
    # global_vars.storage=Storage(store_logs)
    global_vars.storage=Storage([])
    func_id_to_action=dict(map(lambda t:(t[1],t[0]),action_to_func_id.items()))
    count=0
    print("begin")
...
all_pc_cnts=set()
# @profile
def get_best_tx(vm,name,action_to_func_id,action_to_args,store_logs,func_id_to_action):
    """Fake eos transfer vulnerability analysis function.
    Args:
        name: the name of current contract
        vm: WebAssembly module execution environment
    ...
    for action_name in action_to_func_id.keys():
        ...
        try:
            for i,ta in enumerate(test_argument_type):
                if ta=='string' or ta=="struct" or 'checksum' in ta or 'vector' in ta or '[' in 'ta' or ']' in ta or ta=='account_name' or ta=='asset':
                    params[i+1]=utils.gen_symbolic_value(bin_format.i32,f'addr_{i+1}')
                if ta=='string' or ta=='account_name':
                    tas=test_argument_dict[test_argument_name[i]]
                    #经过O3优化后，结构是一个长度（*2）的字节和后面的数据，有个0
                    symbolic_memory.store(params[i+1],1,number.MemoryStore.pack_i8(len(tas)*2))
                    symbolic_memory.store(params[i+1]+1,len(tas),list(map(ord,tas)))
                    #string结构是4字节指针，4字节size，4字节capacity
                    # str_p=utils.gen_symbolic_value(bin_format.i32,f'str_ptr_{i+1}')
                    # symbolic_memory.store(params[i+1],4,number.MemoryStore.pack_i32(str_p))
                    # symbolic_memory.store(params[i+1]+4,4,number.MemoryStore.pack_i32(len(tas)))
                    # symbolic_memory.store(params[i+1]+8,4,number.MemoryStore.pack_i32(len(tas)))
                    # symbolic_memory.store(str_p,len(tas),list(map(ord,tas)))
        except:
            continue
        global_vars.test_argument_type=test_argument_type
        global_vars.test_argument=[test_argument_dict[i] for i in test_argument_name]
        try:
            ...
            states=[]
        # all_states.extend([s for s in states if s.write_storage])
        print("len of pc traces",len(global_vars.pc_cnts))
        for s in states:
            s.func_id=func_id
        all_states.extend(states)
    print("size of states",len(all_states))
    gain_to_gstates = defaultdict(list)
    for i,t in enumerate(all_states):
        s=set(t.pc_traces)
        gain=len(s-global_vars.pc_sets)
    ...
    for g in sorted(gain_to_gstates.keys(),key=lambda k:-k):
        if g<=0:
            ...
            return None
        for ti in sorted(gain_to_gstates[g],key=lambda k:-len(all_states[k].pc_traces)):
            solver=z3.Solver()
            solver.add(all_states[ti].constraints)#这里的约束是根据路径来
            if solver.check() == z3.unsat:
                continue
            model=solver.model()
            print("new trace",all_states[ti].pc_traces)
            for i in all_states:
                all_pc_cnts|=set(i.pc_traces)
                # global_vars.pc_sets|=set(i.pc_traces)
            print("sym coverage")
            print(len(global_vars.pc_cnts),time.time()*1000)
            print(len(all_pc_cnts),time.time()*1000)
            global_vars.pc_sets|=set(all_states[ti].pc_traces)
            print("real coverage")
            print(len(global_vars.pc_sets),time.time()*1000)
            func_id=all_states[ti].func_id
            params=func_id_to_params[func_id]
            action_name=func_id_to_action[func_id]
            test_argument_str,test_argument_type,test_argument_name=action_to_args[action_name]
            test_argument_dict=json.loads(test_argument_str)
            print("path constraints",all_states[ti].constraints)
            for i,p in enumerate(params[1:]):
                if test_argument_type[i]=='string' or test_argument_type[i]=='struct':
                    continue
                val=model.eval(p.n)
                print("eval value",i,val,p.n)
                if utils.is_bv_concrete(val):
                    if test_argument_type[i]=='name':
                        test_argument_dict[test_argument_name[i]]=utils.eos_int_to_abi(utils.get_concrete_int(val))
                    else:
                        test_argument_dict[test_argument_name[i]]=utils.get_concrete_int(val)
                else:
                    if test_argument_type[i]=='name':
                        test_argument_dict[test_argument_name[i]]='bob'
                    else:
                        # test_argument_dict[test_argument_name[i]]=utils.get_concrete_int(val)
                        pass
...
all_pc_cnts=set()
# @profile
def get_rnd_tx(vm,name,action_to_func_id,action_to_args,store_logs,func_id_to_action):
    """Fake eos transfer vulnerability analysis function.
    Args:
        name: the name of current contract
        vm: WebAssembly module execution environment
